...
export const ShoppingCartProvider = ({ children }) => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:4000/users/showAllFromCartToUser")
      .then((res) => {
  ...
  const addToCart = (product) => {
    axios
      .post("http://localhost:4000/cart/addProductToCart", {
  ...
  const increaseQuantity = (product_id) => {
    axios
      .post("http://localhost:4000/cart/modifyCartQuantityToCart", {
        product_id,
        quantity: 1,
      })
      .then(() =>
