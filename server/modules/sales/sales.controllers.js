...
class SalesControllers {
  ...
  getAll = async (req, res) => {
    try {
      let sql =
        'SELECT sale.sale_id, sale.user_id, sale.product_id, sale.sale_status, sale.quantity, sale.date, user.name, user.lastname, product.title,  sale_status, date FROM sale JOIN user ON sale.user_id = user.user_id JOIN product ON sale.product_id = product.product_id WHERE sale.is_deleted = 0 ORDER BY date DESC';
